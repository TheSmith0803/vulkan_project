...
VkResult CreateDebugUtilsMessengerEXT(
    ...
    //casting return of vkGetInstanceProcAddr to proper function pointer type
    if (func != nullptr) {
...
private:
    ...
    void createInstance() {
        //ensure validation layers were setup correctly
        ...
        //this is to check all optional extensions
        uint32_t optExtensionCount = 0;
    ...
    void createSurface() {
        ...
        if (vkCreateWin32SurfaceKHR(instance, &createInfo, nullptr, &surface) != VK_SUCCESS) {
            throw std::runtime_error("Failed to create a window surface! (WIN32SurfaceKHR)");
        }
    ...
    int rateDeviceSuitability(VkPhysicalDevice device) {
        ...
        if (!swapChainAdequate) {
            throw std::runtime_error("GPU does not have appropriate swapchain support!");
        }
    ...
    void pickPhysicalDevice() {
        ...
        if (deviceCount == 0) {
            throw std::runtime_error("Failed to find GPUs with (B)Vulkan support!");
        }

        std::vector<VkPhysicalDevice> devices(deviceCount);
        vkEnumeratePhysicalDevices(instance, &deviceCount, devices.data());

        //ordered map to automatically sort physical device candidates
    ...
    void createLogicalDevice() {
        ...
        //can influence scheduling of command buffer execution with this number
        ...
        vkGetDeviceQueue(device, indices.presentFamily.value(), 0, &presentQueue); //not sure why this is here but it is...
